...
## Hotfixワークフローのルール
   ...
   **masterブランチにのみ存在する問題の修正時：**
   ...
   ### 2. develop→main反映時のエラー修正
   **develop→mainのプルリクエストで発生したCI/CDエラーの修正時：**
   1. **developブランチから fix/xxx 形式でブランチを切る**
   2. CI/CDエラー修正作業を実施（lint、テスト修正等）
   3. **fixブランチを developにマージするプルリクエストを作成**
   4. **developにマージ後、再度 develop→main のプルリクエストを実行**

   **Hotfix対象ケース:**
   - 本番環境での致命的バグ（masterブランチ固有）
   - セキュリティ脆弱性の緊急修正（masterブランチ固有）
   - GitHub Actions ワークフロー修正（masterブランチ固有）

   **Fix対象ケース（develop→main反映時）:**
   - CI/CDエラー（lint、テスト失敗等）
   - ビルドエラー
   - 型チェックエラー

   **重要**: develop→mainの反映で発生したエラーは必ずdevelopブランチから修正し、通常のワークフローに従う
