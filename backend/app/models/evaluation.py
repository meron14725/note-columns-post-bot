...
class Evaluation(BaseModel):
    ...
    quality_score: int = Field(
        ..., ge=0, le=40, description="Writing quality score (0-40)"
    )
    originality_score: int = Field(
        ..., ge=0, le=30, description="Originality score (0-30)"
    )
    entertainment_score: int = Field(
        ..., ge=0, le=30, description="Entertainment score (0-30)"
    )
    total_score: int = Field(..., ge=0, le=100, description="Total score (0-100)")
    ai_summary: str = Field(..., description="AI-generated summary")
    is_retry_evaluation: bool = Field(
        False, description="Whether this is a retry evaluation"
    )
    original_evaluation_id: Optional[int] = Field(
        None, description="Original evaluation ID for retry"
    )
    ...
    def __post_init__(self) -> None:
        ...
        calculated_total = (
            self.quality_score + self.originality_score + self.entertainment_score
        )
...
class AIEvaluationResult(BaseModel):
    ...
    evaluation_reason: Optional[str] = None
    retry_metadata: Optional[dict[str, Any]] = Field(
        None, description="Retry evaluation metadata"
    )
