...
class NoteScraper:
    ...
    def _get_session_tokens(self, url: str) -> bool:
        ...
        try:
            ...
            if not ccd_match:
                # Alternative pattern
                ccd_match = re.search(
                    r'window\.__INITIAL_STATE__.*?"ccd":"([a-f0-9]{64})"', html
                )
            ...
            if not self.xsrf_token:
                logger.info(
                    "No XSRF-TOKEN found in cookies (this is normal for current note.com)"
                )
    ...
    async def collect_articles(self) -> list[Article]:
        ...
        for ref in article_list:
            try:
                article_ref = ArticleReference(
                    ...
                    urlname=ref.get(
                        "urlname", ref["id"]
                    ),  # Use id as fallback for urlname
                ...
                logger.warning(
                    f"Failed to create ArticleReference for {ref.get('id', 'unknown')}: {e}"
                )
                continue
        ...
        logger.info(
            f"Converted {len(articles)} unique articles from {len(article_list)} references"
        )
        return articles
    ...
    async def _collect_list_from_source(
        ...
        logger.info(
            f"Collecting article list from {url_config['name']} (category: {category})"
        )
    ...
    async def _fetch_api_article_list(
        ...
        for page in range(1, max_pages + 1):
            try:
                ...
                # Rate limiting
                await asyncio.sleep(
                    self.collection_settings.get("request_delay_seconds", 1.0)
                )
    ...
    def _parse_api_note_reference(
        self, note: dict[str, Any], category: str
        ...
        try:
            ...
            if price > 0 or not can_read:
                logger.info(
                    f"Excluding paid article: '{title}' (price: ¥{price}, can_read: {can_read})"
                )
                return None
            ...
            if publish_at_str:
                try:
                    ...
                    published_at = datetime.strptime(
                        publish_at_str, "%Y-%m-%dT%H:%M:%S.%f%z"
                    )
                except:
                    try:
                        # Parse as naive datetime and assume UTC
                        naive_dt = datetime.strptime(
                            publish_at_str[:19], "%Y-%m-%dT%H:%M:%S"
                        )
    ...
    async def _fetch_page_article_list(
    ...
    def _filter_recent_article_list(
            ...
            try:
                ...
                if "window.__INITIAL_STATE__" in html:
                    ...
                    if articles:
                        ...
                        logger.warning(
                            f"No articles parsed from __INITIAL_STATE__ in {url}"
                        )
                else:
    ...
    def _parse_note_initial_state(self, html: str, category: str) -> list[Article]:
        ...
        try:
            # Extract JSON data from __INITIAL_STATE__
            start = html.find("window.__INITIAL_STATE__") + len(
            ...
            if "notes" in data:
                ...
                if "contents" in search_results:
                    notes_data = {
                        item["id"]: item for item in search_results["contents"]
                    }

            if not notes_data:
                logger.warning(
                    f"No notes data found in __INITIAL_STATE__ for category {category}"
                )
                return articles
            ...
            logger.info(
                f"Parsed {len(articles)} articles from __INITIAL_STATE__ for category {category}"
            )
    ...
    def _parse_note_item(
        ...
        try:
            ...
            if price > 0 or not can_read:
                logger.info(
                    f"Excluding paid article from note item: '{title}' (price: ¥{price}, can_read: {can_read})"
                )
                return None
            ...
            if "publishAt" in item:
                try:
                    published_at = datetime.fromisoformat(
                        item["publishAt"].replace("Z", "+00:00")
                    )
                except:
                    pass
    ...
    async def _parse_html_response(self, html: str, category: str) -> list[Article]:
        ...
        article_elements = soup.find_all(
            ["article", "div"], class_=re.compile(r"note|article|post")
        )
    ...
    async def _parse_html_article_element(
        ...
        try:
            ...
            title_element = element.find(
                ["h1", "h2", "h3", "a"], class_=re.compile(r"title|heading")
            )
            ...
            author_element = element.find(
                ["span", "div"], class_=re.compile(r"author|user|creator")
            )
            author = (
                author_element.get_text(strip=True) if author_element else "Unknown"
            )
    ...
    def _filter_recent_articles(self, articles: list[Article]) -> list[Article]:
        ...
        return [
            article for article in articles if article.published_at >= threshold_date
        ]
    ...
    def _fetch_article_detail(self, urlname: str, key: str) -> Optional[dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.warning(
                    f"Failed to fetch article detail: {response.status_code}"
                )
                return None
    ...
    def _parse_article_detail_from_initial_state(
        self, html: str, key: str
    ) -> Optional[dict[str, Any]]:
        ...
        try:
            ...
            if not note:
                logger.warning(
                    f"Could not find note with key {key} in __INITIAL_STATE__"
                )
                return None
            ...
            if price > 0 or not can_read:
                logger.info(
                    f"Excluding paid article from initial state: '{note.get('name', 'Unknown')}' (price: ¥{price}, can_read: {can_read})"
                )
                return None
            ...
            if publish_at_str:
                try:
                    published_at = datetime.fromisoformat(
                        publish_at_str.replace("Z", "+00:00").replace("+09:00", "+0900")
                    )
                except:
                    try:
                        # Parse as naive datetime and assume UTC
    ...
    def _parse_article_detail_from_nuxt(
        self, html: str, key: str
    ) -> Optional[dict[str, Any]]:
    ...
    def _parse_article_detail_from_html(
        ...
        try:
            ...
            if (
                abs((published_at - current_time).total_seconds()) < 60
            ):  # If still close to current time
    ...
    @log_execution_time
    async def collect_article_with_details(
        self, urlname: str = None, key: str = None
    ) -> Optional[Article]:
        ...
        try:
            ...
            article = Article(
                id=str(
                    detail.get("id", key)
                ),  # Use detail ID if available, otherwise key
