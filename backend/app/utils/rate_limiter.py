...
class RateLimiter:
    ...
    def _wait_if_needed(self) -> Optional[float]:
        ...
        if self.rate_limit.requests_per_second:
            recent_requests = sum(
                1 for req_time in self.request_times if current_time - req_time < 1
            )
...
# Add common services
rate_limiter.add_service(
    "groq", RateLimit(requests_per_minute=30, requests_per_day=14400)
)

rate_limiter.add_service(
    "note",
    RateLimit(requests_per_minute=60, requests_per_day=5000, requests_per_second=2),
)

rate_limiter.add_service(
    "twitter", RateLimit(requests_per_minute=300, requests_per_day=2000)
)
