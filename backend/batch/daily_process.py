...
from backend.app.utils.logger import log_execution_time, setup_logger
from config.config import config, ensure_directories, validate_required_env_vars
...
class DailyBatchProcessor:
    ...
    async def run_daily_batch(self) -> bool:
        ...
        try:
            ...
            logger.info(
                "Daily batch process completed successfully (streaming evaluation)"
            )
            return True
        ...
        try:
            ...
            async with NoteScraper() as scraper:
                ...
                try:
                    ...
                    logger.info(
                        f"Found {len(existing_ids)} existing articles in database"
                    )
                except Exception as e:
                ...
                for i, ref in enumerate(new_article_refs):
                    try:
                        ...
                        if not article_detail:
                            logger.warning(
                                f"  ✗ Failed to fetch details for {ref['key']}"
                            )
                            continue
                        ...
                        delay = config.get_collection_settings().get(
    ...
    def _log_completion_stats(self) -> None:
        ...
        try:
            ...
            if eval_stats.get("total", 0) > 0:
                logger.info(
                    f"Average score today: {eval_stats.get('average_total_score', 0):.1f}"
                )
                logger.info(
