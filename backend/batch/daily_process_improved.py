...
from backend.app.utils.logger import setup_logger, log_execution_time, get_logger
...
class ImprovedDailyBatchProcessor:
    ...
    def __init__(self, target_categories: Optional[List[str]] = None, limit: Optional[int] = None) -> None:
        ...
        logger.info(f"Initialized processor (categories: {target_categories}, limit: {limit})")
    ...
    async def run_daily_batch(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Daily batch completed successfully: {evaluated_count} articles evaluated")
            return True
    ...
    async def _collect_and_evaluate_with_references(self) -> int:
        ...
        """
        logger.info("Step 1: Improved collection and evaluation with reference deduplication")
        ...
        try:
            ...
            async with NoteScraper() as scraper:
                ...
                logger.info("Phase 1: Collecting article references from all categories...")
                ...
                logger.info(f"Found {len(article_list)} article references across all categories")
                ...
                for ref in article_list:
                    try:
                        ...
                        logger.warning(f"Failed to create ArticleReference for {ref.get('key', 'unknown')}: {e}")
                        continue
                ...
                if self.target_categories:
                    ...
                    unprocessed_refs = [ref for ref in unprocessed_refs if ref.category in self.target_categories]
                    logger.info(f"Filtered from {original_count} to {len(unprocessed_refs)} references for categories: {self.target_categories}")
                ...
                logger.info(f"Found {len(unprocessed_refs)} article references for processing")
                ...
                for i, ref in enumerate(unprocessed_refs):
                    try:
                        logger.info(f"[{i+1}/{len(unprocessed_refs)}] Processing: {ref.title[:50]}...")
                        ...
                        if saved_count > 0:
                            ...
                            if evaluation:
                                ...
                                if eval_saved > 0:
                                    ...
                                    self.article_ref_repo.mark_as_processed(ref.key, ref.urlname)
                                    evaluations_count += 1
                        ...
                        if (i + 1) % 10 == 0:
                            logger.info(f"Progress: {i + 1}/{len(unprocessed_refs)} articles processed, {evaluations_count} evaluations completed")
                    
                    except Exception as e:
                        logger.error(f"Failed to process article reference {ref.key}/{ref.urlname}: {e}")
                        continue
...
async def main():
    ...
    parser.add_argument("--json-only", action="store_true", help="Generate JSON files only")
    parser.add_argument("--categories", nargs="*", help="Process specific categories only")
